...
const Navbar = () => {
  ...
  return (
    <nav className=" fixed top-0 left-0 right-0  opacity-1 text-white z-50 ">
          ...
          <div className="hidden sm:flex justify-center items-center sm:space-x-4">
            {navLinks.map((link) => (
              <Link key={link.name} href={link.path}>
                <span className="hover:bg-gradient-to-l hover:from-cyan-500 hover:to-blue-500 px-3 py-2 rounded-full text-lg font-semibold ">
                  {link.name}
                </span>
              </Link>
            ))}
      ...
      {isOpen && (
        <div
          className="sm:hidden bg-gradient-to-br from-black via-gray-800 to-gray-900 backdrop:blur-md rounded-xl "
          id="mobile-menu"
        >
              ...
              >
                <Link key={link.name} href={link.path}>
                  <motion.li
                    ...
                    className="block hover:bg-gray-700 px-3 py-2 text-center rounded-full text-lg font-bold"
                  >
                    {link.name}
                  </motion.li>
                </Link>
